...
public class AddTerm extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		idGet_Mode = iAddTerm.getIntExtra("id_Mode", 0); // mode 0: add, mode 1:
															// edit
		...
		if (savedInstanceState != null) {
			...
			Date eDate = new Date();
			try {
				sDate = format.parse(cTerm.getString(cTerm
						.getColumnIndex("termStartDate")));
			} catch (ParseException e) {
				e.printStackTrace();
			}

			try {
				eDate = format.parse(cTerm.getString(cTerm
						.getColumnIndex("termEndDate")));
			} catch (ParseException e) {
				e.printStackTrace();
			}
		...
		if (idGet_Mode == 1) {
			etTermTitle.setText(cTerm.getString(cTerm
	...
	private DatePickerDialog.OnDateSetListener mSDateSetListener = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker view, int year, int monthOfYear,
			...
			selectedSDate = mSYear + "-" + twoDigit.format(mSMonth + 1) + "-"
					+ twoDigit.format(mSDay);
			bTermPickStartDate.setText(mSYear + "-"
	...
	private DatePickerDialog.OnDateSetListener mEDateSetListener = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker view, int year, int monthOfYear,
			...
			selectedEDate = mEYear + "-" + twoDigit.format(mEMonth + 1) + "-"
					+ twoDigit.format(mEDay);
			bTermPickEndDate.setText(mEYear + "-"
		...
		if (etTermTitle.getText().toString().trim().equals("")) {
			...
			if (mSYear > mEYear
				...
				if (idGet_Mode == 0) {
					...
					termsDB.open();
					termsDB.updateTerm(idEditGet_Item, termTitle,
